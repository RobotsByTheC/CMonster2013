...
import org.usfirst.frc2084.robotsimulator.wpilibj.util.AllocationException;
import org.usfirst.frc2084.robotsimulator.wpilibj.util.CheckedAllocationException;
...
/**
 * Class implements the PWM generation in the FPGA. Values supplied as arguments
 * for PWM outputs range from -1.0 to 1.0. They are mapped to the hardware
 * dependent values, in this case 0-255 for the FPGA. Changes are immediately
 * sent to the FPGA, and the update occurs at the next FPGA cycle. There is no
 * delay.
 *
 * As of revision 0.1.4 of the FPGA, the FPGA interprets the 0-255 values as
 * follows: 255 = full "forward" 254 to 129 = linear scaling from "full forward"
 * to "center" 128 = center value 127 to 2 = linear scaling from "center" to
 * "full reverse" 1 = full "reverse" 0 = disabled (i.e. PWM output is held low)
 */
public class PWM extends SensorBase implements LiveWindowSendable {

    private static Resource allocated = new Resource((2 * kPwmChannels));
     ...
     *
     * - 20ms periods (50 Hz) are the "safest" setting in that this works for
     * all devices - 20ms periods seem to be desirable for Vex Motors - 20ms
     * periods are the specified period for HS-322HD servos, but work reliably
     * down to 10.0 ms; starting at about 8.5ms, the servo sometimes hums and
     * get hot; by 5.0ms the hum is nearly continuous - 10ms periods work well
     * for Victor 884 - 5ms periods allows higher update rates for Luminary
     * Micro Jaguar speed controllers. Due to the shipping firmware on the
     * Jaguar, we can't run the update period less than 5.05 ms.
     *
     ...
     *
     * - There are 128 pwm values less than the center, so... - The minimum
     * output pulse length is 1.5ms - 128 * 6.525us = 0.665ms - 0.665ms /
     * 6.525us per tick = 102
     */
    ...
    private void initPWM(final int moduleNumber, final int channel) {
        ...
        m_channel = channel;
        m_module = (DigitalModule) cRIO.getInstance().getDigitalModule(moduleNumber);
    ...
    /**
     * Allocate a PWM given a module and channel. Allocate a PWM using a module
     * and channel number.
     *
