...
/* the project.                                                               */
/*----------------------------------------------------------------------------*/
package org.usfirst.frc2084.robotsimulator.wpilibj.command;
...
import org.usfirst.frc2084.robotsimulator.wpilibj.DriverStation;
import org.usfirst.frc2084.robotsimulator.wpilibj.NamedSendable;
import org.usfirst.frc2084.robotsimulator.wpilibj.Timer;
 ...
 *
 * <p>However, if at any point while it is running
 * {@link Command#cancel() cancel()} is called, then the command will be stopped
 * and {@link Command#interrupted() interrupted()} will be called.</p>
 *
 * <p>If a command uses a {@link Subsystem}, then it should specify that it does
 * so by calling the {@link Command#requires(Subsystem) requires(...)} method in
...
public abstract class Command implements NamedSendable {
     ...
     * command. This method is crucial to the functioning of the Command System
     * in general.
     *
     ...
     * @throws IllegalUseOfCommandException if this command has started before
     * or if it has been given to a {@link CommandGroup}
     * @see Subsystem
     */
     ...
     * {@link Command#interrupted() interrupted()} or
     ...
     *
     * <p>It may be useful for a team to reference the
     * {@link Command#isTimedOut() isTimedOut()} method for time-sensitive
     * commands.</p>
     *
    ...
    /**
     * Called to indicate that the timer should start. This is called right
     * before {@link Command#initialize() initialize()} is, inside the
     ...
     * {@link Command#timeSinceInitialized() timeSinceInitialized()} method
     * returns a number which is greater than or equal to the timeout for the
     * command. If there is no timeout, this will always return false.
     *
     ...
     * the command is running though, then the command will be marked as
     * canceled and eventually removed.</p>
     * <p>A command can not be canceled if it is a part of a command group, you
     * must cancel the command group instead.</p>
     *
     * @throws IllegalUseOfCommandException if this command is a part of a
     * command group
     */
     ...
     *
     * @return the {@link CommandGroup} that this command is a part of (or null
     * if not in group)
     */
     ...
     * disabled.
     *
     * <p>By default a command will not run when the robot is disabled, and will
     * in fact be canceled.</p>
     *
